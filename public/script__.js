...
const TipManager = {
    ...
    send() {
        ...
        if (text.length > 250) {
            UIManager.showPostVoteMessage("Keep it short! Under 250 chars.");
            return;
        }
...
const ContactManager = {
    // We can reuse your existing Service/Template IDs so you don't need to configure new ones!
    serviceID: 'service_b6d75wi',   
    templateID: 'template_qody7q7', 
    ...
    init() {
        ...
        el.innerHTML = `
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl">
                ...
                <textarea id="contactInput" rows="5" class="w-full p-3 border border-gray-300 rounded-lg mb-4 focus:ring-2 focus:ring-indigo-500 focus:outline-none resize-none" placeholder="Write your message here..."></textarea>
    ...
    send() {
        ...
        if (!text) {
            UIManager.showPostVoteMessage("Please write a message first!");
            return;
        }
        ...
        // 3. Send in background (Reusing your Tip template)
        // Note: The email you receive will still say "User Tip:", but you'll know it's a contact message based on the context.
        emailjs.send(this.serviceID, this.templateID, {
            message: "CONTACT: " + text, // We add a prefix so you know it's a contact msg
            username: State.data.username || "Anonymous"
        }).catch((err) => {
            console.error('Background email failed:', err);
        });
