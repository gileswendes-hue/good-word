...
inner.style.transform = `rotate(${Math.random() * 360}deg)`;
        ...
        requestAnimationFrame(() => {
           wrap.style.left = startLeft ? '-150px' : '110vw'; // Destination
        ...
        // 2. 10% Chance to turn back
        if (Math.random() < 0.10 && fishEmoji !== '🐙' && fishEmoji !== '🥾') {
           const retreatDelay = (duration * 1000) * 0.4; // Stop at 40% of way

           setTimeout(() => {
             if (!wrap.parentNode) return; // If already caught, do nothing
...
if (fishEmoji === '🐙') {
        e.stopPropagation(); // Prevent standard catch
        
        // 1. Update Stats & Badges manually since we are returning early
        if (data.k) State.unlockBadge(data.k);
        ...
        // 3. Create Ink Trail (Cloud of particles)
        ...
        for (let i = 0; i < 12; i++) {
          ...
          // Randomize ink position slightly to form a cloud
        ...
        // 4. Jet Forward Animation
        // Determine "forward" based on current direction (baseDir)
        // baseDir 1 = moving right, -1 = moving left.
        // We overwrite the current transition to make it fast.
        wrap.style.transition = 'left 0.4s cubic-bezier(0.25, 1, 0.5, 1), top 0.4s ease-out';
        
        // Move rapidly off-screen in the direction of travel
        const jetTo = baseDir === 1 ? '120vw' : '-30vw';
        wrap.style.left = jetTo;
        
        // Remove the octopus after it jets away
        setTimeout(() => {
            if(wrap.parentNode) wrap.remove();
        }, 400);

        return; // Stop standard pop/catch logic
...
c.appendChild(wrap);
      ...
      requestAnimationFrame(() => {
        if (isBoot) {
          wrap.style.top = '-20%'; // Boot floats up
        } else {
          ...
          if (Math.random() < 0.10) {
            const retreatDelay = (duration * 1000) * 0.4; // Interrupt at 40% of the journey

            setTimeout(() => {
              if (!wrap.parentNode) return; // Stop if already caught
              ...
              // 3. Wait a moment, then turn back
              setTimeout(() => {
                if (!wrap.parentNode) return;

                // Flip the fish graphic to face the other way
                // baseDir is the original direction (-1 or 1). We invert it.
                inner.style.transform = `scaleX(${-baseDir})`;
              ...
              }, 800); // 0.8s pause to read the text
      ...
      // Cleanup: Remove element after animation ends
      // We add extra time (+2000ms) to ensure the 'retreating' fish has time to leave the screen
      setTimeout(() => {
        if (wrap.parentNode) wrap.remove();
      }, duration * 1000 + 2000);
...
const PinPad = {
  ...
  init() {
    ...
    // Using existing tailwind styles from your file
    el.innerHTML = `
  ...
  shakeBox() {
    ...
    if (box) {
      ...
      box.classList.add('animate-shake'); // Requires shake animation in CSS
    }
  },
  ...
  isLocked() {
    ...
    // If lockout expired, we can reset attempts (optional, but cleaner)
