...
import { Game } from './main.js'; // Imported to allow calling Game methods from events
    ...
    init() {
        ...
        if (DOM.inputs.settings.tilt) {
            DOM.inputs.settings.tilt.onchange = e => {
                ...
                // We need to import TiltManager or trigger it. 
                // Since this file can't import effects.js directly due to circular dependency (effects imports ui),
                // we rely on the TiltManager singleton being available globally or handle it via a custom event, 
                // OR simpler: imported TiltManager via a re-export if possible.
                // For now, we will assume TiltManager is refreshed via game loop or simply imported dynamically if needed.
                // However, since we split files, we can just use an event or import it if no circular dependency exists.
                // Effects.js imports UI.js. UI.js cannot import Effects.js top level.
                // SOLUTION: Use dynamic import or dispatch event.
